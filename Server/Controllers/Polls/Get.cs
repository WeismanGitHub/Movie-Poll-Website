...
public class GetController: ControllerBase {
	...
	public async Task<IActionResult> GetPoll([FromRoute] Guid id, Settings settings) {
		...
		Movie[] movies = await Task.WhenAll(poll.MovieIds.Select(async (id) => {
			HttpResponseMessage res = await client.GetAsync($"https://api.themoviedb.org/3/movie/{id}");
			...
			TMDbMovieResponse? movie = JsonSerializer.Deserialize<TMDbMovieResponse>(await res.Content.ReadAsStringAsync());
